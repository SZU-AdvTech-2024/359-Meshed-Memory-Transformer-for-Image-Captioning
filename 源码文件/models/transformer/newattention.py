...
class ScaledDotProductAttention(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h):
        ...
        self.fc_q = nn.Linear(d_model, h * d_k) #d_model 也是embeding size
    ...
    def init_weights(self):
        nn.init.xavier_uniform_(self.fc_q.weight)  #权重初始化
        nn.init.xavier_uniform_(self.fc_k.weight)
        nn.init.xavier_uniform_(self.fc_v.weight)
        nn.init.xavier_uniform_(self.fc_o.weight)
        nn.init.constant_(self.fc_q.bias, 0) #把bias 设置为0
    ...
    def forward(self, queries, keys, values, attention_mask=None):
        ...
        # querie = self.fc_q(queries)
        att = torch.matmul(q, k) / np.sqrt(self.d_k)  # (b_s, h, nq, nk)
        if attention_mask is not None:
            att = att.masked_fill(attention_mask, -np.inf)
        att = torch.softmax(att, -1)
        out = torch.matmul(att, v)
        out = out.permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v) #矩阵乘法，和V相乘
        # out = out * self.gate(querie)
...
class ScaledDotProductAttention2(nn.Module):
    ...
    def forward(self, queries,keys, values,dprev,attention_mask=None):
        '''
        Computes
        :param queries: Queries (b_s, nq, d_model)
        :param keys: Keys (b_s, nk, d_model)
        ...
        v = self.fc_v(values).view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, h, nk, d_v)
        att = torch.matmul(q, k) / np.sqrt(self.d_k)  # (b_s, h, nq, nk)
        # dprev1 =torch.matmul(q, v1) / np.sqrt(self.d_v)
...
class MultiHeadAttention2(Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
        ...
        if attention_module is not None:
            ...
            self.attention = ScaledDotProductAttention2(d_model=d_model, d_k=d_k, d_v=d_v, h=h)
    ...
    def forward(self, queries, keys, values, dprev,attention_mask=None):
        ...
        if self.identity_map_reordering:
            ...
            out,dprev = self.attention(q_norm, k_norm, v_norm,dprev,attention_mask)
            out =queries + self.dropout(torch.relu(out))
        else:
            out,dprev = self.attention(queries,keys, values,dprev,attention_mask)
